...
public class CustomerRangeValidator : AbstractValidator<List<CustomerBase>>
{
    public CustomerRangeValidator(AbstractValidator<CustomerBase> customerValidator)
    {
        RuleFor(p => p.ToList()).Custom((information, custom) =>
        {
            ...
            foreach (var item in information)
            {
                ...
                {
                    if (item.Email.ToString().Length < 256)
                    {
